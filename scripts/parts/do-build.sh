...
if [ "$ENABLE_DEBUG" = "on" ]; then
  (cd src/${PDT_NAME}; sed -i "s/^set(CMAKE_BUILD_TYPE\sRelease)$/set(CMAKE_BUILD_TYPE Debug)/g" CMakeLists.txt)
fi
(mkdir build/${PDT_NAME}; cd build/${PDT_NAME}; cmake -DBUILD_TESTING=1 -DBUILD_SHARED_LIBS=on -DCMAKE_INSTALL_PREFIX=$INSTALL_PREFIX ../../src/${PDT_NAME};  make; make -j 2; make install;)
